...
const ResetPassword = ({ setView, authCallbackError }) => {
    ...
    const handleUpdatePassword = async (e) => {
        ...
        try {
            const { data: { session } } = await supabase.auth.getSession();
            if (!session) {
                throw new Error('Session expired. Request a new reset link from the login page.');
            }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4">
            <div className="max-w-md w-full bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-10 border-2 border-amber-200 dark:border-amber-900/50">
                <div className="text-center mb-10">
                    <div className="w-20 h-20 bg-amber-500 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl shadow-amber-500/20">
                ...
                {success ? (
                    <div className="text-center">
                        ...
                        <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-3">Password Updated</h3>
                    ...
                    <div className="text-center space-y-6">
                        ...
                        <button
                            ...
                            className="w-full bg-amber-600 hover:bg-amber-700 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-sm"
                        >
                    ...
                    <>
                        ...
                        <form onSubmit={handleUpdatePassword} className="space-y-6">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-amber-600 hover:bg-amber-700 text-white font-black py-4 rounded-2xl shadow-xl shadow-amber-500/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-sm disabled:opacity-70"
                            >
